...
**Expected:** `bin.glissa` points at `bin/glissa.js`, `engines.node` is `>=22.16.0`, and `files` matches the array above (check `package.json` directly for the current list; it grows as new server-side modules ship).
