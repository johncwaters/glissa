...
Audit at `30bd4c6`: FIXED V3 (`521f1b7`, no bare Write allow), V8 (`521f1b7`, sha256 marker), M1
(`05fe2b6`, `previous/` fallback), M6 (`45c12c3`, consumer-gated service). OPEN: V1, V2, V4, V5, V6,
V7, V9, M2, M3, M4, M5, M7, M8, M9. Ordered by exploitability then blast radius, one commit each:

1. V1 + V2 (`server/visions-dispatch.js:101`, `session/adapters/claude-code.js:207`,
...
2. M2 (`server/pack-builder.js:270`): `clearStaleTmpDirs` deletes any process's tmp dir. Per-pack
...
3. M3 (`server/core/pack-core.js:592,684,795`): two skill dirs sharing a basename overwrite.
   Refuse duplicate final `relPath` in `planPackBuild`, publishing nothing.
4. V5 + V6 (`server/visions-wiring.js:502,629,675,681`, `server/visions-dispatch.js:67`): carry the
...
5. V7 (`server/visions-wiring.js:61,201`, `session/visions-relay.js:201,215`): the relay resets its
...
6. V9 (`server/visions-wiring.js:202,209,896`): `didClose` from one relay wipes shared per-uri
   state. Track owners per uri; clear on the last close.
7. M9 (`server/pack-distiller.js:47,97`): `dangerouslySkipPermissions` plus bare `Edit(*)`/`Write(*)`
...
8. M5 + M8 (`server/pack-builder.js:396,429,440`, `server/pack-service.js:134,168`): one throwing
...
9. V4 (`server/core/visions-dispatch-core.js:103,169`): `hashText('')` is truthy so the
   `empty-document` gate never fires. Gate on length; whitespace-only counts as empty.
10. M4 (`server/pack-builder.js:58,91,178`): a leading `**` yields an empty root and zero matches.
    Anchor to `baseDir` for both the walk and the watch root.
11. M7 (`session/sessions.js:1059,2433`): `pack-updated` landing mid-resolution finds no delivered
