...
| Hook | Fired? | Notes |
|---|---|---|
| `UserPromptSubmit` | ✅ t≈9.7s | payload `permission_mode: "bypassPermissions"`, `session_id`, `cwd` |
| `Stop` | ✅ t≈14s | **fires under bypassPermissions** — the COMPLETE signal works in YOLO |
| `SessionEnd` | ✅ t≈16s | lifecycle |
| `SessionStart` | ❌ (in `-p`) | did not fire in print mode; verify in interactive (US-009) |
| `Notification` | ❌ (in `-p`) | idle/permission notifications are interactive-only; verify in US-009 |
...
- **`Stop` misses turns that end on a tool call** (#3118, interactive). ⇒ pair `Stop` with
  `Notification(idle_prompt)`; either reaching the session means "done."
- **`Stop` can double-fire** (#3465). ⇒ StatusSource dedup window.
- **HTTP-hook default timeout is 600s, no retry, blocks Claude's loop.** ⇒ inject a short timeout
  (~2–5s) and make the `/hook` handler record-and-return-200 immediately.
- **Managed/enterprise settings outrank `--settings`** (`disableAllHooks`/`allowManagedHooksOnly`)
  and could silently block injected hooks (realistic in the ELM corporate env). ⇒ startup guard:
  if no hook callback arrives for a known-working session, fall back to OSC-title + a visible
