...
A poke spends the SAME `maxPerHour` budget an edit does. The gate once carved a separate `activityMaxPerHour` quota out of it, with an edit-vs-activity trigger classification and an `armedBy` cold-start tie-break; that ladder was deleted in the 2026-08-22 navigator simplification pass (see docs/plan-navigator.md, "What was removed") because one cap decides the same thing. What survives is the movement signal itself: an untouched buffer re-opens only when the ingest seq advanced, so an aging digest can never buy a dispatch, and the per-document cooldown bounds how often a busy machine can spend the budget.
