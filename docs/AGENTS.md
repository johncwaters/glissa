...
| `plan-plan-review.md` | Draft plan (2026-09-07) for reviewing agent plans from the dashboard: the `ExitPlanMode` PermissionRequest hook already carries the full plan, so Glissa holds that reply on a second hook URL, while a dashboard or phone client is connected, and the operator reads the whole plan on desktop or phone and approves, edits, or sends section-addressed feedback through the hook decision, with the terminal dialog as the fail-open fallback |
