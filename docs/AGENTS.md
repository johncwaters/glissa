...
| `plan-visions-3.md` | Plan for Visions long-term memory: agent-agnostic machine-global store (HMAC-signed segmented JSONL canon + distilled markdown projection), multi-vendor transcript ingestion, memory-distill lane, fenced prompt + direct-read delivery (pack carrier deferred to M17), milestones M12-M17, revised after a three-reviewer pass, nothing shipped |
