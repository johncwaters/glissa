/*
 * Usage tracking wiring - the IO shell that binds server/usage-scanner.ts (pure-ish, fs injected) and
 * server/usage-pricing.ts to real Sessions, the dashboard broadcast, and the control-WS request.
 *
 * Same shape as server/posthog-wiring.ts: createBackend calls createUsageWiring once with its live
...
// Loaded lazily, because session/core/rtk-command.ts pulls in spawn-command, which resolves `claude` at
// module load, and control-handlers.ts imports this module on a path that has no business probing for it.
 ...
 * Defensive rather than strict: control-handlers.ts validateUsage REJECTS a bad value on the way in,
...
function createUsageWiring({
  ...
  // Read-only view of the lane ledger; the writes happen on the hook callback path in backend.ts.
  laneMap = null,
