...
/**
 * `tick` is the lane's tick body; `writeState` persists whatever the lane calls its state. Both are
 * injected, so the loop stays IO-free like the pollers it serves.
 *
 * A tick body may report a failed poll by returning `{ failed: true, retryAfterMs? }`, which opens a
 * full-jitter backoff window (server/core/lane-backoff.ts) that later ticks are SKIPPED inside. The
