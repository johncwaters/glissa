...
// session/core/agent-tracker.js): no sockets, no timers, no IO. The backend registers one key per
 ...
 * per-connection).
 * @param {boolean[]} focusStates one entry per open control connection
 */
function decideFocusSuppression(focusStates) {
 ...
 * notification is FOR.
 */
function decideOffDashboardDelivery(connectionCount) {
  return connectionCount === 0;
}

function createClientPresence() {
  const focusedByConnection = new Map(); // connection key -> its last reported focus
...
module.exports = { createClientPresence, decideFocusSuppression, decideOffDashboardDelivery };
