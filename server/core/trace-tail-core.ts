...
const MAX_SUBAGENT_CHUNKS_PER_STOP = 64;
const TRACE_TAIL_SCAN_BYTES = MAX_TRANSCRIPT_READ_BYTES;
...
function withCommittedOffset(
  ...
  offset: number,
  { maxRemembered = MAX_REMEMBERED_TRANSCRIPTS }: { maxRemembered?: number } = {},
  ...
  const rememberedLimit = Math.max(1, wholeNumber(maxRemembered, MAX_REMEMBERED_TRANSCRIPTS));
  const overflow = rememberedPaths.length - rememberedLimit;
  if (overflow <= 0) return remembered;
  for (const stalePath of rememberedPaths.slice(0, overflow)) delete remembered[stalePath];
  return remembered;
}

function committedOffsetFromTraceTailOrNull(tailText: string, {
  transcriptPath,
  pathBeforeWindow = null,
  isWholeFile = false,
}: { transcriptPath: string; pathBeforeWindow?: string | null; isWholeFile?: boolean }): number | null {
  ...
  let hasSeenTargetRun = pathOfCurrentRun === transcriptPath;
  let tracedOffset = 0;
  for (const record of records) {
    ...
    hasSeenTargetRun = true;
    tracedOffset = Math.max(tracedOffset, wholeNumber(record.transcriptOffset, 0));
  }
  if (!hasSeenTargetRun) return null;
  return tracedOffset;
}

function resumeOffsetFrom(
  checkpoint: { transcriptPath?: unknown; offset?: unknown; offsetByTranscriptPath?: unknown } | null | undefined,
  { transcriptPath, size, alreadyTracedOffset = 0, traceSize = 0 }:
    { transcriptPath: string; size: number; alreadyTracedOffset?: number | null; traceSize?: number },
): TraceResumePoint {
  const transcriptSize = wholeNumber(size, 0);
  const cannotEstablishRun = !checkpoint && alreadyTracedOffset === null && wholeNumber(traceSize, 0) > 0;
  if (cannotEstablishRun) {
    return { offset: transcriptSize, didReset: false, didFallbackToTranscriptEnd: true };
  }
  ...
  if (committed === 0) return { offset: 0, didReset: false, didFallbackToTranscriptEnd: false };
  if (committed > transcriptSize) return { offset: 0, didReset: true, didFallbackToTranscriptEnd: false };
...
function isOversizedPartialLine(
  carry: string | Buffer,
  { maxPartialLineBytes = MAX_PARTIAL_LINE_BYTES } = {},
): boolean {
  const carryBytes = Buffer.isBuffer(carry) ? carry.length : Buffer.byteLength(carry, 'utf8');
  return carryBytes > Math.max(1, Math.floor(maxPartialLineBytes));
}
