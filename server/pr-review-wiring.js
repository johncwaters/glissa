...
function buildReviewPrompt({ slug, number, baseRefName, conflicting, resultPath }) {
  ...
  const lines = [
    ...
    '- Open with a 2-3 sentence plain-language summary: the verdict, how many blocking findings, and their common theme.',
    '- Group findings under "### Blocking" and "### Non-blocking" headings; omit a heading that has no findings.',
    '- Number each finding. Start it with a bold one-line title stating the problem in plain words, then the location as an inline-code `path:line`.',
    '- Keep the evidence under each finding to at most 3 short sentences, then end with a "Fix:" line giving the concrete change.',
    '- Prefer short sentences and blank lines over dense prose; never write a paragraph longer than 3 sentences.',
    ...
    '- A closing "Clean elsewhere" note, if any, is one sentence.',
    '',
    'Steps:',
