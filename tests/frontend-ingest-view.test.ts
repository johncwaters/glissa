...
function event(seq: number, overrides: WireActivityEvent = {}): WireActivityEvent {
  return {
    source: 'terminal',
    kind: 'output',
    ts: NOW,
...
test('a delta with nothing in it leaves the standing list alone', async () => {
  ...
  const emptyDelta = { events: [], overflow: 40 };
  assert.deepEqual(applyActivityMessage(standing, emptyDelta).map((e) => e.seq), [1]);
});
...
test('the rendered list is bounded, however long the tab is left open', async () => {
  ...
  for (let batch = 0; batch < 30; batch += 1) {
    const events: WireActivityEvent[] = [];
