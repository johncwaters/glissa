...
import {
  ...
  isSpinnerChar,
  isKnownIdleChar,
} from '../detection/osc-title-source.ts';
import type { TitleSignal } from '../detection/osc-title-source.ts';

function bulletTitle(char: string) {
  return `\x1b]0;${char} Claude Code\x07`;
}

function collect(src: ReturnType<typeof createOscTitleSource>): TitleSignal[] {
