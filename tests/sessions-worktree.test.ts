...
type PtySpawnOptions = Parameters<NonNullable<SessionOptions['ptySpawn']>>[2];
import { capturingPty, exitablePty, fakePty } from './helpers/fake-pty.ts';
import type { PtyExitEvent } from './helpers/fake-pty.ts';
...
interface FakeWorkspaceOptions {
  ...
  mergeKeepResult?: MergeResult;
}

function fakeGitWorkspace(opts: FakeWorkspaceOptions = {}) {
  const calls = {
    create: [] as CallArgs[],
    detectDefaultBranch: [] as CallArgs[],
    syncIntegrationBranch: [] as CallArgs[],
    rebaseOnly: [] as CallArgs[],
    mergeBack: [] as CallArgs[],
    mergeKeep: [] as CallArgs[],
    discard: [] as CallArgs[],
    populate: [] as CallArgs[],
    hasUnmergedWork: [] as CallArgs[],
    sequence: [] as string[],
  };
  ...
  return {
    ...
    mergeBack(args: CallArgs) { calls.mergeBack.push(args); return opts.mergeResult || { merged: true }; },
    mergeKeep(args: CallArgs) {
      calls.mergeKeep.push(args);
      const workspace = args.workspace as Workspace;
      const kept: MergeResult = { merged: true, branch: workspace.branch, base: String(args.targetBranch), baseSha: 'newbase' };
      return opts.mergeKeepResult || kept;
    },
    discard(args: CallArgs) { calls.discard.push(args); },
  };
}

function stubSignature(session: Session, extra: CallArgs = {}) {
...
const rebaseDecisions = (session: Session) => session.getDebugState().decisions.filter((d) => d.kind === 'rebase');

function makeSession(extra: Partial<SessionOptions> = {}) {
  return new Session({
    id: 'wt-sess',
    name: 'wt-sess',
    path: process.cwd(),
...
function attachWorktree(session: Session, worktreeDir: string, { baseSha = null, mergeStatus = 'none' }: { baseSha?: string | null; mergeStatus?: MergeStatus } = {}) {
  session.worktreeLifecycle.adoptWorktree({
    worktreeDir,
    branch: 'feat',
    base: 'develop',
    baseSha,
  ...
  session.worktreeLifecycle.setMergeStatus(mergeStatus, {}, { emit: false });
}

function setMergeStatus(session: Session, mergeStatus: MergeStatus) {
  session.worktreeLifecycle.setMergeStatus(mergeStatus, {}, { emit: false });
}
...
test('a vanished spawn cwd fails terminally and reaps the live PTY', async () => {
  ...
  const s = makeSession({
    ...
    signalProc(pid, signal) {
      ...
      const error: NodeJS.ErrnoException = new Error('missing process');
      error.code = 'ESRCH';
      throw error;
    },
    ptySpawn: () => ({
      ...fakePty(),
      onExit(handler: (event: PtyExitEvent) => unknown) {
  ...
  try {
    ...
    assert.equal(typeof exit.handler, 'function');
    await exit.handler?.({ exitCode: 0 });
    assert.equal(s.state, STATES.FAILED);
...
test('a diverged auto-detected base forks locally and emits a warning', async () => {
  ...
  try {
    ...
    assert.equal(warnings[0]?.notice, 'base main has diverged from origin/main; forked from local');
  } finally {
    session.destroy();
    fs.rmSync(wt, { recursive: true, force: true });
  }
...
test('start() BLOCKS when the integration branch is missing - stays DORMANT, no spawn, notice set', async () => {
  ...
  s.on('worktree-blocked', (e) => { events.blocked = e; });
  try {
    await s.start();
    ...
    assert.ok(events.blocked && events.blocked.branch === 'develop', 'worktree-blocked event emitted');
...
test('start() ADOPTS its own surviving worktree on branch-in-use (failed boot removal / discard)', async () => {
  ...
  s.on('worktree-blocked', (e) => { events.blocked = e; });
  s.on('worktree-ready', (e) => { events.ready = e; });
  try {
    await s.start();
    ...
    assert.equal(events.blocked, null, 'no worktree-blocked event');
    assert.ok(events.ready && events.ready.worktreeDir === wt, 'worktree-ready announces the adopted dir');
    ...
    assert.ok(s.worktreeLifecycle.snapshot().hasPendingCheck, 'worktree check armed after the watcher (re)start');
  } finally {
    s.destroy();
    fs.rmSync(wt, { recursive: true, force: true });
  }
...
test('start() runs in place with a notice when the branch-in-use conflict dir is gone (unadoptable)', async () => {
  ...
  try {
    ...
    assert.ok(events.blocked && events.blocked.notice === s.worktreeNotice, 'worktree-blocked event carries the same notice');
  } finally {
    s.destroy();
  }
});
...
test('a fresh restart waits for the pending kill reap before it touches the worktree', async () => {
  ...
  try {
    ...
    s._killReap = new Promise<void>((resolve) => { releaseReap = resolve; });
...
test('a fresh restart destroyed during the integration sync never rebases', async () => {
  ...
  gw.syncIntegrationBranch = async (args) => {
    const result = await engineSync(args);
    if (armed) holder.session?.destroy();
    return result;
  };
  const s = makeSession({ gitWorkspace: gw, integrationBranch: 'develop', ptySpawn: () => fakePty() });
  holder.session = s;
...
test('a fresh restart waits for the natural-exit reap before it touches the worktree', async () => {
  ...
  try {
    ...
    assert.ok(([STATES.DONE, STATES.FAILED] as SessionState[]).includes(s.state), 'the natural exit settled the session');
...
test('mergeWorktree delegates to the engine and clears the worktree on success', async () => {
  const wt = realWorktreeDir();
  const gw = fakeGitWorkspace({ worktreeDir: wt, mergeResult: { merged: true } });
...
test('getDiff: committed range + gate come from the integration branch, not a stale baseSha', { skip: !GIT }, async () => {
  ...
  try {
    attachWorktree(s, repo, { baseSha: git(['rev-parse', 'develop'], repo).trim() });
    const d = await s.getDiff();
...
test('getDiff: a branch already on develop shows nothing to merge despite a stale baseSha (out-of-band merge)', { skip: !GIT }, async () => {
  ...
  const s = makeSession({ integrationBranch: 'develop' });
  try {
    const staleBase = git(['rev-parse', 'develop'], repo).trim();

    git(['checkout', 'develop'], repo);
    git(['merge', '--ff-only', 'feat'], repo);
    git(['checkout', 'feat'], repo);
    attachWorktree(s, repo, { baseSha: staleBase });
    const d = await s.getDiff();
...
test('resyncBranch: behind + a DIFFERENT branch checked out -> ff-fetch (never touches the working tree)', { skip: !GIT }, async () => {
  const { dir: repo, remoteDir } = initRepoWithRemote();
  git(['checkout', '-q', '-b', 'other-branch'], repo);
  const clone = pushRemoteOnlyCommit(remoteDir, 'develop', 'remote-only.txt');
  try {
    const s = makeSession({ integrationBranch: 'develop' });
    s.path = repo;
...
function advanceDevelop(repo: string) {
  git(['checkout', 'develop'], repo);
...
test('resetToDormant: returns to DORMANT only when settled (PTY dead + no worktree), else no-op', async () => {
  ...
  }
  {
    ...
    try {
      s.state = STATES.DONE;
      attachWorktree(s, '/tmp/some-worktree');
      setMergeStatus(s, 'pending-review');
      const ok = s.resetToDormant();
...
test('_settleWorktreeOnExit: a COMMITTED but clean worktree is kept for review, never discarded', async () => {
  ...
  try {
    ...
    assert.equal(gw.calls.hasUnmergedWork[0].projectPath, s.path);
    assert.equal((gw.calls.hasUnmergedWork[0].workspace as Workspace).cwd, wt);
    assert.equal(gw.calls.hasUnmergedWork[0].integrationBranch, 'develop');
  } finally { s.destroy(); fs.rmSync(wt, { recursive: true, force: true }); }
});
...
test('teardown mutex is symmetric: finishAndMerge and restart mutually exclude', async () => {
  {
    ...
    try {
      ...
      s.mergeWorktree = async () => ({ merged: true });
      s.ptyProcess = null; s.emit('exit', { exitCode: 0 }); await drain();
    } finally { s.destroy(); }
  }
  {
    const s = makeSession();
...
test('start(): destroy() during the provision await -> no spawn', async () => {
  ...
  const createHeld = new Promise<void>((r) => { releaseCreate = r; });
  let spawns = 0;
  const gw = {
    calls: { create: [] as CallArgs[] },
    async create(args: CallArgs): Promise<Workspace> {
...
test('mergeWorktree re-entry: a second call while merging is refused, engine invoked once', async () => {
  ...
  try {
    await s.start();
    const p1 = s.mergeWorktree();
    const r2 = await s.mergeWorktree();
...
test('_handlePtyExit: a rejecting settle still emits "exit" and clears the teardown flag (no deadlock)', async () => {
  ...
  try {
    ...
    gw.discard = () => Promise.reject(new Error('settle boom'));
    let exited = false;
    s._finishing = true;
...
test('_handlePtyExit after destroy(): tree kept, "exit" still emitted, kill reap still settles', async () => {
  ...
  try {
    ...
    await Promise.allSettled([s._killReap]);
  } finally { fs.rmSync(wt, { recursive: true, force: true }); }
});
...
test('finishAndMerge settled-branch mutex: a double-click is refused, engine merges once', async () => {
  ...
  try {
    ...
    s.state = STATES.DONE;
    setMergeStatus(s, 'pending-review');
    const r1 = s.finishAndMerge();
    assert.equal(r1.ok, true);
    const r2 = s.finishAndMerge();
...
async function captureUnhandled(fn: () => unknown) {
