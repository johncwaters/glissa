...
import type { DocumentRange } from '../server/core/visions-rules-core.ts';
import { sweepMarkdown, sweepMarkdownWithFixes } from '../server/core/visions-rules-core.ts';
import {
  AUTO_SAFE_CODES,
  appendFixLog,
  autoSafeFixes,
  ...
  rangesOverlap,
  readSweepResult,
} from '../server/core/visions-fix-core.ts';

const REPEAT_TEXT = '# Title\n\nA line with with a repeat.\n';
const FENCE_TEXT = '# Title\n\n```js\nconst answer = 42;\n';

function applyEdit(text: string, editRange: DocumentRange, newText: string) {
