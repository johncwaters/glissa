...
import { mapHookToSignal } from '../detection/hook-source.ts';
import { buildHookSettings, WAKEUP_TOOL_MATCHER, HOOK_EVENTS } from '../detection/settings-injector.ts';
import * as wakeupTracker from '../session/core/wakeup-tracker.ts';
...
test('ScheduleWakeup populates pendingWakeup and emits wakeup-change; no transition', () => {
  ...
  assert.equal(Number(pw?.at) > Date.now() + 290 * 1000, true, 'fireAt approx now + delaySeconds');
