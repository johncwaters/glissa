...
import { MAX_PROMPT_BYTES } from '../server/core/visions-dispatch-core.ts';
import { DIGEST_BUDGET_CHARS, createVisionsWiring, VISIONS_DEBOUNCE_MS } from '../server/visions-wiring.ts';
import type { VisionsWiringOptions } from '../server/visions-wiring.ts';
import { listenOnLoopback, reserveFreePort } from './helpers/http-server.ts';

type Wiring = ReturnType<typeof createVisionsWiring>;
type Connection = ReturnType<Wiring['openConnection']>;
type DispatchFn = NonNullable<VisionsWiringOptions['dispatch']>;
type DispatchArgs = Parameters<DispatchFn>[0];
type DispatchOutcome = Awaited<ReturnType<DispatchFn>>;
...
function asFrame(message: unknown): LaneFrame {
  if (!isRecord(message) || typeof message.type !== 'string') throw new Error('the lane sent a frame with no type');
  return { ...message, type: message.type };
}

function frameAt(frames: LaneFrame[], index: number): LaneFrame {
  const frame = index < 0 ? frames.at(index) : frames[index];
  if (!frame) throw new Error(`no frame at ${index}`);
  return frame;
}
...
function frameDiagnostic(frame: LaneFrame | null, index: number): Diagnostic {
  if (!frame) throw new Error('no frame to read a diagnostic from');
  const diagnostic = diagnosticsOf(frame)[index];
  if (!diagnostic) throw new Error(`no diagnostic at ${index}`);
  return diagnostic;
}

function diagAt(frames: LaneFrame[], index: number): Diagnostic {
  const diagnostic = diagnosticsOf(frames.at(-1))[index];
  if (!diagnostic) throw new Error(`no diagnostic at ${index}`);
  return diagnostic;
}
...
function diagnosticsOf(frame: LaneFrame | undefined): Diagnostic[] {
  const diagnostics = paramAt(frame, 'diagnostics');
  if (!Array.isArray(diagnostics)) throw new Error('the frame carries no diagnostics array');
  return diagnostics;
}

const MARKDOWN_URI = 'file:///tmp/plan-visions.md';
const HARNESS_SCOPE = [{ id: 'harness', path: '/tmp' }];
...
function fakeTimers() {
  ...
  return {
    ...
    get pendingCount() { return pendingByHandle.size; },
    get pendingIds(): NodeJS.Timeout[] { return [...pendingByHandle.keys()]; },
  };
}
...
function snapshotAt(wiring: Wiring, index: number) {
  const entry = wiring.documentsSnapshot()[index];
  if (!entry) throw new Error(`no document snapshot at ${index}`);
  return entry;
}
...
interface CodeAction {
  ...
  edit?: { documentChanges?: DocumentChange[] };
}

function actionAt(actions: CodeAction[], index: number): CodeAction {
  const action = actions[index];
  if (!action) throw new Error(`no code action at ${index}`);
  return action;
}
...
function documentChangesOf(edit: { documentChanges?: DocumentChange[] } | undefined): DocumentChange[] {
  const changes = edit?.documentChanges;
  if (!Array.isArray(changes)) throw new Error('the edit carries no documentChanges');
  return changes;
}
...
function requestIdOf(frame: LaneFrame | undefined): string {
  const id = frame?.id;
  if (typeof id !== 'string') throw new Error('the request carries no id');
  return id;
}
...
function callAt(calls: DispatchArgs[], index: number): DispatchArgs {
  const args = index < 0 ? calls.at(index) : calls[index];
  if (!args) throw new Error(`no dispatch at ${index}`);
  return args;
}
...
function drivenConnection(options: VisionsWiringOptions = {}) {
  const timers = fakeTimers();
  const warnings: string[] = [];
  const notes: string[] = [];
  const sent: LaneFrame[] = [];
  const broadcasts: LaneFrame[] = [];
  ...
  const wiring = createVisionsWiring({
    ...
    broadcast: (message) => { broadcasts.push(asFrame(message)); },
    scopeProjects: HARNESS_SCOPE,
    ...options,
  });
  const connection = wiring.openConnection({ send: (message) => { sent.push(asFrame(message)); } });
  const lsp = (method: string, params: unknown) => connection.handleFrame(JSON.stringify({ type: 'lsp', method, params }));
...
function countingFsPromises() {
  ...
  return {
    ...
    fsPromises: {
      ...fs.promises,
      writeFile: (...args: Parameters<typeof fs.promises.writeFile>) => {
        writes.push(args);
...
function didOpenParams(uri: string, languageId: string | undefined, text: string) {
...
function rangedChangeParams(uri: string, version: number, contentChanges: unknown[]) {
...
test('a didChange carrying ranges is applied and sweeps the spliced text', (t) => {
  ...
  timers.runPending();
  assert.deepEqual(diagnosticsOf(frameAt(sent, 0)), [], 'the opened text is clean');
  ...
  assert.deepEqual(diagnosticsOf(frameAt(sent, 1)).map((d) => d.code), ['repeated-word'], 'the sweep read the spliced text');
...
test('a malformed range is refused with the uri, version, change index and range in the line', (t) => {
  ...
  assert.match(String(warnings[0]), /ignored textDocument\/didChange: invalid-range/);
  assert.match(String(warnings[0]), /uri=file:\/\/\/tmp\/plan-visions\.md/);
  assert.match(String(warnings[0]), /version=7/);
  assert.match(String(warnings[0]), /change=0/);
  assert.match(String(warnings[0]), /range=0:0-\?/);
...
test('the didChange and debounced-sweep lines are debug-gated and name sizes rather than text', (t) => {
  ...
  assert.match(String(changeLine), new RegExp(`v2 \\(1 changes, ${secret.length} chars\\)`));
...
test('a stale didChange names both versions in the line it logs', (t) => {
  ...
  assert.equal(warnings.length, 1);
  assert.match(String(warnings[0]), /stale-version \(uri=.+ incoming=4 current=9\)/);
});
...
test('a burst of markdown edits coalesces into one sweep of the final text', (t) => {
  ...
  assert.equal(frameAt(sent, 0).type, 'publishDiagnostics');
  assert.equal(paramAt(frameAt(sent, 0), 'uri'), MARKDOWN_URI);
  assert.deepEqual(diagnosticsOf(frameAt(sent, 0)).map((d) => d.code), ['repeated-word']);
});
...
test('two open documents debounce independently', (t) => {
  ...
  timers.runPending();
  assert.deepEqual(sent.map((msg) => paramAt(msg, 'uri')).sort(), [MARKDOWN_URI, otherUri].sort());
});
...
test('a sweep that publishes also broadcasts the findings for that uri', (t) => {
  ...
  assert.equal(frameAt(broadcasts, 0).type, 'visions-findings');
  assert.equal(frameAt(broadcasts, 0).uri, MARKDOWN_URI);
  assert.equal(frameAt(broadcasts, 0).ts, FIXED_TS, 'the ts comes from the injected clock');
  assert.deepEqual(broadcastDiagnostics(frameAt(broadcasts, 0)), diagnosticsOf(frameAt(sent, 0)), 'the tab sees what the editor sees');
});
...
function dispatchingConnection({
  dispatch: overrides = {}, respond = null, contextDigest = null, contextSeq = null, scopeProjects = HARNESS_SCOPE, debug = false,
  ...wiringOptions
}: DispatchingOptions = {}) {
...
test('a dispatch fires one quiet window after a sweep publishes, carrying the buffer and its findings', async (t) => {
  ...
  assert.equal(callAt(calls, 0).uri, MARKDOWN_URI);
  assert.equal(callAt(calls, 0).text, REPEATED_WORD_MARKDOWN);
  assert.deepEqual(findingCodes(callAt(calls, 0)), ['repeated-word'], 'tier 2 rides along');
});
...
test('an edit restarts the quiet window rather than letting the armed one run out', async (t) => {
  ...
  await wiring.whenDispatchSettled();
  assert.equal(calls.length, 1);
  assert.equal(callAt(calls, 0).text, edited, 'the dispatch reads the buffer as it stands when the window expires');
});
...
test('a boundary reached while a dispatch is in flight is gated, not queued', async (t) => {
  ...
  const { wiring, timers, calls, notes, lsp } = dispatchingConnection({
    ...
    respond: (_args, callNumber) => (callNumber === 1 ? held.promise : Promise.resolve({ verdict: 'NONE', comments: [] })),
  });
  t.after(() => wiring.stop());
  ...
  held.settle({ verdict: 'COMMENTS', comments: [COMMENT], reason: null });
  await inFlight;
});
...
test('model diagnostics publish and broadcast as a union after rule diagnostics', async (t) => {
  ...
  assert.deepEqual(diagnosticsOf(published.at(-1)).map((diagnostic) => diagnostic.code), ['repeated-word', 'model']);
  assert.equal(diagAt(published, 1).range?.end.character, '# Title'.length);
  assert.equal(diagAt(published, 1).severity, 4);
  const findings = broadcasts.filter((message) => message.type === 'visions-findings');
  assert.deepEqual(broadcastDiagnostics(frameAt(findings, -1)), diagnosticsOf(published.at(-1)));
...
test('a dispatch comment reaches the editor and survives the next keystroke', async (t) => {
  ...
  const withComment = diagnosticsOf(sent.filter((message) => message.type === 'publishDiagnostics').at(-1));
...
test('shared uri state survives until its last owner closes', async (t) => {
  ...
  const secondSent: LaneFrame[] = [];
  const secondConnection = harness.wiring.openConnection({ send: (message) => { secondSent.push(asFrame(message)); } });
  const secondLsp = (method: string, params: unknown) => secondConnection.handleFrame(JSON.stringify({ type: 'lsp', method, params }));
  ...
  assert.equal(resultRows(codeActions).length, 1);
...
test('connection close clears shared uri state only when its owner is last', (t) => {
  ...
  assert.equal(broadcasts.filter((message) => message.type === 'visions-findings' && broadcastRowCount(message, 'diagnostics') === 0).length, 0);

  secondConnection.close();

  assert.deepEqual(findingSections(wiring), [], 'the last owner clears the shared section');
  assert.equal(broadcasts.filter((message) => message.type === 'visions-findings' && broadcastRowCount(message, 'diagnostics') === 0).length, 1);
});
...
test('a second didClose after clearing shared uri state is tolerated', (t) => {
  ...
  timers.runPending();
  lsp('textDocument/didClose', { textDocument: { uri: MARKDOWN_URI } });
  const clearBroadcasts = broadcasts.filter((message) => message.type === 'visions-findings' && broadcastRowCount(message, 'diagnostics') === 0);
...
test('a result that lands after its buffer closed is dropped rather than resurrecting a section', async (t) => {
  const held = deferredOutcome();
  const { wiring, timers, broadcasts, lsp } = dispatchingConnection({ respond: () => held.promise });
  t.after(() => wiring.stop());
...
test('an edit during dispatch carries comments diagnostics hand and intent to their new lines', async (t) => {
  ...
  wiring.applyModelIntent('the standing intent', 'harness', MARKDOWN_URI);
  openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  runSweepThenDispatch(timers);
  const inFlight = wiring.whenDispatchSettled();
  ...
  assert.equal(wiring.getIntentFor('harness', MARKDOWN_URI).active.text, 'a moved intent');
...
test('a wholesale replacement during dispatch still drops comments diagnostics hand and intent', async (t) => {
  ...
  assert.equal(wiring.getIntentFor('harness', MARKDOWN_URI).active.text, 'the standing intent');
...
test('a hand is broadcast only when it changes and joins the connect-time snapshot', async (t) => {
  ...
  const republished = diagnosticsOf(sent.filter((message) => message.type === 'publishDiagnostics').at(-1));
  assert.ok(
...
test('a hand raised with nothing else reaches the editor in its own frame', async (t) => {
  ...
  const published = sent.filter((message) => message.type === 'publishDiagnostics' && paramAt(message, 'uri') === MARKDOWN_URI);
  const diagnostics = diagnosticsOf(published.at(-1));
...
function threadShape(thread: { id: string }, { text, uris = [], ts = FIXED_TS, hits = 1 }: {
  text: string;
  uris?: string[];
  ts?: number;
  hits?: number;
...
test('an object proposal opens or names a thread, and an unknown id is refused as a count only', (t) => {
  ...
  assert.ok(storyA, 'story A opened a thread');
...
test('a corrupt intent file starts empty and warns once', (t) => {
  ...
  assert.deepEqual(wiring.getIntent(), { byProject: {}, unowned: [] });
  assert.equal(warnings.length, 1);
  assert.match(String(warnings[0]), /intent state unreadable, starting empty/);
});
...
test('without an intent path the lane keeps the same in-memory behavior and touches no storage', async (t) => {
  ...
  const fsPromises = {
    ...
    appendFile: () => Promise.reject(new Error('must not write')),
  };
...
test('the standing intent rides the dispatch, and the result advances it after the comments', async (t) => {
  ...
  wiring.applyModelIntent('an early guess', 'harness', MARKDOWN_URI);
  openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  runSweepThenDispatch(timers);
  await wiring.whenDispatchSettled();
...
test('a dispatch result may open a second thread, and the next prompt names the other one', async (t) => {
  ...
  wiring.applyModelIntent('the first story', 'harness');
  openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  runSweepThenDispatch(timers);
  await wiring.whenDispatchSettled();
  assert.equal(wiring.getIntentFor('harness', MARKDOWN_URI).threads.length, 2);
...
test('an ERROR result cannot move the standing intent', async (t) => {
  ...
  wiring.applyModelIntent('the accepted belief', 'harness', MARKDOWN_URI);
  const beforeDispatch = intentBroadcasts(broadcasts).length;
  openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  runSweepThenDispatch(timers);
  await wiring.whenDispatchSettled();

  assert.equal(wiring.getIntentFor('harness', MARKDOWN_URI).active.text, 'the accepted belief');
...
test('a dispatch on an owned uri reads only that project, and an unowned thread never reaches its prompt', async (t) => {
  ...
  assert.equal(callAt(calls, 0).intent, '', 'a project with no thread delivers no intent line, and never the unowned text');
  assert.equal(String(callAt(calls, 0).prompt).includes('the unowned belief'), false);
  assert.equal(String(callAt(calls, 0).prompt).includes('Current working intent'), false);
  ...
  assert.equal(frameAt(intentBroadcasts(broadcasts), -1).projectId, PROJECT_ID);
});
...
test('a second dispatch on the same project reads the project thread it opened', async (t) => {
  ...
  assert.equal(calls.length, 2);
  assert.equal(callAt(calls, 1).intent, 'what this project is really for');
});
...
function sendRequest(connection: Connection, id: string, method: string, params: unknown): void {
  connection.handleFrame(JSON.stringify({
    type: 'lsp-request', id, method, params,
  }));
}

function answerRequest(connection: Connection, id: string, result: unknown): void {
  connection.handleFrame(JSON.stringify({ type: 'lsp-response', id, result }));
}

function requestCodeActions(harness: DrivenHarness, { id = 'ca-1', uri = MARKDOWN_URI, range = null }: {
  id?: string;
  uri?: string;
  range?: unknown;
} = {}): CodeAction[] {
  ...
  if (!Array.isArray(result)) throw new Error(`no code action answer for ${id}`);
  return result;
}

function applyEditRequests(harness: DrivenHarness): LaneFrame[] {
  return harness.sent.filter((message) => message.type === 'lsp-request' && message.method === 'workspace/applyEdit');
}

function fixBroadcasts(harness: DrivenHarness): LaneFrame[] {
...
test('a code action is answered from what the last sweep computed, never from a fresh sweep', (t) => {
  ...
  assert.equal(actionAt(actions, 0).kind, 'quickfix');
  assert.equal(actionDiagnostic(actionAt(actions, 0)).code, 'repeated-word');
  assert.deepEqual(documentChangesOf(actionAt(actions, 0).edit)[0]?.textDocument, { uri: MARKDOWN_URI, version: 1 });
});
...
test('a selection is answered with the fixes it touches and nothing else', (t) => {
  ...
  assert.deepEqual(onLineFour.map((action) => actionDiagnostic(action).code), ['repeated-word']);
  assert.equal(actionDiagnostic(actionAt(onLineFour, 0)).range?.start.line, 4);
});
...
test('with autoFix on one sweep asks for one versioned edit carrying only the auto-safe fixes', (t) => {
  ...
  const [change] = documentChangesOf(requestedEdit(frameAt(requests, 0)));
  assert.ok(change, 'the request carries a document change');
  ...
  assert.equal(paramAt(frameAt(requests, 0), 'label'), 'Visions: 2 silent fixes');
});
...
test('an applied edit is logged and broadcast once per fix, and joins the snapshot', (t) => {
  ...
  openEdited(harness.lsp, MARKDOWN_URI, TWO_REPEATS_AND_A_FENCE);
  harness.timers.runPending();
  answerRequest(harness.connection, requestIdOf(applyEditRequests(harness)[0]), { applied: true });
...
test('a refused edit is logged exactly as loudly and is never retried', (t) => {
  ...
  openEdited(harness.lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  harness.timers.runPending();
  answerRequest(harness.connection, requestIdOf(applyEditRequests(harness)[0]), { applied: false });

  assert.deepEqual(fixBroadcasts(harness).map((message) => fixOf(message).applied), [false]);
...
test('the changelog is capped, so an unattended lane cannot grow it without end', (t) => {
  ...
  for (let round = 0; round < 25; round++) {
    ...
    harness.timers.runPending();
    answerRequest(harness.connection, requestIdOf(applyEditRequests(harness).pop()), { applied: true });
  }
...
async function bootBackend(configPatch: Record<string, unknown>, { remotePort = null }: { remotePort?: number | null } = {}): Promise<BootedBackend & { port: number }> {
  ...
  if (!backend) throw new Error('the backend never booted');
  entry.server.on('request', backend.app);
  const port = await listenOnLoopback(entry.server);
  entry.port = port;

  if (remotePort) {
    const remoteServer = http.createServer();
    entry.remoteServer = remoteServer;
...
function visionsClient(port: number) {
  const ws = new WebSocket(`ws://127.0.0.1:${port}/visions`);
  const frames: LaneFrame[] = [];
  ws.on('message', (data: Buffer) => { frames.push(asFrame(JSON.parse(data.toString()))); });
  ...
  return {
    ...
    frames,
    opened,
    sendRaw: (raw: string) => ws.send(raw),
    sendLsp: (method: string, params: unknown) => ws.send(JSON.stringify({ type: 'lsp', method, params })),
    close: () => ws.close(),
  };
}

async function waitForDiagnostics(client: ReturnType<typeof visionsClient>, uri: string, timeoutMs = 4000): Promise<LaneFrame | null> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const frame = client.frames.find((msg) => msg.type === 'publishDiagnostics'
      && paramAt(msg, 'uri') === uri
      && diagnosticsOf(msg).length > 0);
...
function backendDestroyedUpgrade(server: http.Server, port: number, requestPath: string): Promise<boolean> {
  return new Promise((resolve, reject) => {
    ...
    const socketToServer = net.connect(port, '127.0.0.1', () => {
      socketToServer.write([
...
test('/visions is refused on the remote listener even with the lane enabled', async () => {
  ...
  assert.equal(
    await backendDestroyedUpgrade(requireServer(remoteServer), remotePort, '/visions'), true,
...
test('the digest is read once per dispatch and rides into the prompt builder', async (t) => {
  const digestCalls: { budgetChars: number; now: number }[] = [];
  const { wiring, timers, calls, lsp } = dispatchingConnection({
    contextDigest: (args) => {
  ...
  assert.ok(String(callAt(calls, 0).digest).includes('- terminal 4s ago: npm test'));
});

test('a sweep that never dispatches never asks the ingest lane for a digest', async (t) => {
  let digestReads = 0;
  const { wiring, timers, lsp } = drivenConnection({ contextDigest: () => { digestReads += 1; return 'x'; } });
  t.after(() => wiring.stop());
...
test('activity alone re-dispatches an edited buffer, and the belief it comes back with lands', async (t) => {
  ...
  assert.equal(callAt(calls, 1).text, REPEATED_WORD_MARKDOWN);
  assert.equal(wiring.getIntentFor('harness', MARKDOWN_URI).active.text, 'wiring the ingest lane into the visions gate');
});
...
test('a seq that is not a finite number is read as no lane rather than as movement', async (t) => {
  ...
  } = dispatchingConnection({ dispatch: { cooldownMs: 1 }, contextSeq: () => Number.NaN });
  t.after(() => wiring.stop());
...
test('a raised hand never returns as a standing finding the next prompt says not to repeat', async (t) => {
  ...
  assert.equal(calls.length, 2);
  assert.equal(String(callAt(calls, 1).prompt).includes(HAND), false, 'obeying "do not repeat" here is what lowers the hand');
  assert.equal(String(callAt(calls, 1).prompt).includes(COMMENT.message), false, 'a tier 3 comment is not a standing tier 2 finding either');
...
test('a session-authored ERROR leaves the lane healthy, and a transport failure still backs it off', async (t) => {
  ...
  const drive = async ({ wiring, timers, lsp, clock }: DrivenHarness) => {
    openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
    runSweepThenDispatch(timers);
    await wiring.whenDispatchSettled();
    for (const version of [2, 3, 4]) {
...
test('a freshly opened buffer orients once: intent and hand land, comments and diagnostics never do', async (t) => {
  ...
  assert.equal(wiring.getIntentFor('harness', MARKDOWN_URI).active.text, 'a plan doc about spawning');
  assert.equal(broadcasts.some((message) => message.type === 'visions-comments' && broadcastRowCount(message, 'comments') > 0), false);
...
test('an intent-based comment lands anywhere while a thread stands, and nowhere without one', async (t) => {
  ...
  withThread.wiring.applyModelIntent('the story', 'harness', MARKDOWN_URI);
...
test('touched ranges are per connection and per open, so a second window orients on its own', async (t) => {
  ...
  assert.equal(String(callAt(harness.calls, 0).prompt).includes('orientation pass'), false);
  ...
  assert.match(String(callAt(harness.calls, 1).prompt), /orientation pass/, 'the other window edited nothing');
});
...
test('an orientation leaves the comments, model diagnostics and hand an edit dispatch published standing', async (t) => {
  ...
  assert.match(String(callAt(harness.calls, 1).prompt), /orientation pass/);
...
test('a thread bound to this document is never listed as also in flight elsewhere', async (t) => {
  ...
  wiring.applyModelIntent({ thread: 'new', text: 'the story of this document' }, 'harness', MARKDOWN_URI);
  wiring.applyModelIntent({ thread: 'new', text: 'a second story of this document' }, 'harness', MARKDOWN_URI);
  wiring.applyModelIntent({ thread: 'new', text: 'the story of another document' }, 'harness', 'file:///tmp/other-plan.md');
  openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  runSweepThenDispatch(timers);
  await wiring.whenDispatchSettled();
...
test('a thread that expires while its dispatch runs is retired, and the result opens a fresh one', async (t) => {
  ...
  wiring.applyModelIntent('the belief the dispatch was reading', 'harness', MARKDOWN_URI);
  const expiredId = wiring.getIntentFor('harness', MARKDOWN_URI).active.id;
  openEdited(lsp, MARKDOWN_URI, REPEATED_WORD_MARKDOWN);
  runSweepThenDispatch(timers);
  await wiring.whenDispatchSettled();

  assert.equal(calls.length, 1);
  assert.equal(callAt(calls, 0).intent, 'the belief the dispatch was reading', 'the prompt read the thread while it was live');
  const threads = wiring.getIntentFor('harness', MARKDOWN_URI).threads;
