...
function foundIds(matched: string[] | null): string[] {
  return matched ?? [];
}
...
test('the schema is created idempotently and stamps its shape', () => {
  ...
  try {
    assert.equal(pragmaValue(raw, 'user_version', 'user_version'), SCHEMA_VERSION);
    assert.equal(pragmaValue(raw, 'journal_mode', 'journal_mode'), 'wal');
  } finally {
    raw.close();
  }
});
...
test('the search index is derived, so a count that disagrees with the canon is rebuilt', () => {
  ...
  assert.equal(foundIds(db.searchIds(['poller'], 10)).length, 0, 'the emptied index answers nothing');
...
test('secure_delete is on, which is what makes a freed row unreadable rather than merely unlinked', () => {
  ...
  try {
    assert.equal(pragmaValue(db, 'secure_delete', 'secure_delete'), 1);
    assert.equal(pragmaValue(db, 'journal_mode', 'journal_mode'), 'wal');
  } finally {
    db.close();
  }
});
