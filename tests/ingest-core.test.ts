...
import type { IngestEvent, IngestRing } from '../server/core/ingest-core.ts';
...
import {
  ...
  scrubText,
  snapshotEvents,
} from '../server/core/ingest-core.ts';
...
function terminalEvent(summary: string, extra: Record<string, unknown> = {}) {
  return {
...
function storedCommand(raw: string) {
...
test('a secret in a shell command never reaches a ring entry', () => {
  ...
  assert.ok(!String(stored.detail?.text).includes('sk-live-1234'), String(stored.detail?.text));
...
test('detail strings are scrubbed too, and unsupported detail value types are dropped', () => {
  ...
  const [stored] = snapshotEvents(store);
  assert.ok(String(stored.detail?.text).includes(SCRUB_PLACEHOLDER));
...
test('scope normalizes to the one comparable key, and a missing ts falls back to now', () => {
  const event = normalizeEvent({ source: 'git', kind: 'commit', summary: 'add a thing' }, { seq: 3, now: NOW }) as IngestEvent;
