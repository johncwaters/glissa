...
test('matching versions are never stale', async () => {
  const { stalePackNames } = await importCore();
  assert.deepEqual(stalePackNames([{ name: 'crew-rules', version: 'v9' }], { 'crew-rules': 'v9' }), []);
});
...
test('a session that delivered no packs is never stale', async () => {
  const { stalePackNames } = await importCore();
  assert.deepEqual(stalePackNames([], { 'crew-rules': 'v2' }), []);
  assert.deepEqual(stalePackNames(undefined, { 'crew-rules': 'v2' }), []);
  assert.deepEqual(stalePackNames([{ name: 'crew-rules', version: 'v1' }], null), []);
});
