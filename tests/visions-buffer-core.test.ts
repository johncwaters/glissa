...
function openDoc(store: DocStore, uri: string, text: string, version = 1) {
...
function boundary(previousText: unknown, nextText: unknown, changes: unknown) {
  return detectBlankLineBoundary({ previousText, nextText, changes });
}

function textAfter(store: DocStore, uri: string, contentChanges: unknown[], opened: string) {
