...
const shellArgs = (script) => (isWindows ? ['/c', script] : ['-c', script]);

function spawnReal(script) {
...
const nextTickDelay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
...
function makeRealSession({ script, id = 'pty-real' }) {
  const spawned = [];
  const session = new Session({
    ...
    ptySpawn: (_file, _args, options) => {
      const terminal = pty.spawn(shell, shellArgs(script), options);
...
test('node-pty delegates on/listeners to its own socket, not to a second emitter', { skip: isWindows }, () => {
  const terminal = spawnReal('sleep 30');
  try {
