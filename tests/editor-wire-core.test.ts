...
} from '../server/core/editor-wire-core.ts';

const INVOCATION = { command: 'glimmervoid', args: ['visions', 'relay'] };

test('the neovim drop-in is a whole file glimmervoid owns', () => {
  const text = neovimDropIn(INVOCATION);
  assert.match(text, /Written by glimmervoid/);
  assert.match(text, /vim\.lsp\.start\(\{ name = 'glimmervoid-visions', cmd = \{ 'glimmervoid', 'visions', 'relay' \} \}\)/);
...
test('helix with no config gets the server and a markdown language entry, once', () => {
  ...
  assert.match(first.text, /\[language-server\.glimmervoid-visions\]/);
  assert.match(first.text, /language-servers = \["marksman", "glimmervoid-visions"\]/);
...
test('an existing markdown entry keeps its own servers and settings', () => {
  ...
  assert.match(merged.text, /language-servers = \["marksman", "glimmervoid-visions"\]/);
...
test('an empty language-servers list takes our entry without a stray comma', () => {
  const existing = '[[language]]\nname = "markdown"\nlanguage-servers = []\n';
  const merged = helixMerge(existing, INVOCATION);
  assert.match(merged.text, /language-servers = \["glimmervoid-visions"\]/);
  assert.equal(helixMerge(merged.text, INVOCATION).changed, false);
  assert.equal(helixRemove(merged.text).text.includes('glimmervoid-visions'), false);
});
...
test('an unterminated block is refused by both directions rather than guessed at', () => {
  ...
  const brokenEmacs = ';; >>> glimmervoid-visions\n(with-eval-after-load \'eglot)\n';
...
test('an appended block keeps one blank line from the file it joined', () => {
  ...
  assert.equal(merged.text.endsWith(';; <<< glimmervoid-visions\n'), true);
  assert.equal(emacsMerge(merged.text, INVOCATION).changed, false);
...
test('json settings gain one key and lose exactly that key again', () => {
  ...
  assert.deepEqual(parsed.clients['glimmervoid-visions'].command, ['glimmervoid', 'visions', 'relay']);
  ...
  assert.equal(JSON.parse(removed.text).clients['glimmervoid-visions'], undefined);
});

test('a settings file glimmervoid cannot parse is left untouched', () => {
