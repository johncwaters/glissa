...
test('no idle/silence timer or content scraping remains - sessions.ts AND the mapper (fail-closed)', () => {
  ...
  // readFileSync throws on a bad path, so a moved/renamed target FAILS CLOSED (never silently passes).
...
test('a quiet drain releases only after the settle window elapses, with detail.deferred true', (t) => {
  t.mock.timers.enable({ apis: ['setTimeout', 'Date'] });
  const s = makeSession(STATES.RUNNING, { gateReleaseSettleMs: 30 });
  // A holder, because TypeScript keeps the initializer narrowing on a `let` only a callback assigns.
