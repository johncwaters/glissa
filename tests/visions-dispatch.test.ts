...
import {
  ...
  readCommentsResult,
  visionsPermissions,
} from '../server/visions-dispatch.ts';
import type { VisionsSpawn } from '../server/visions-dispatch.ts';
import { MAX_PROMPT_BYTES, buildVisionsPrompt } from '../server/core/visions-dispatch-core.ts';

type DispatcherOptions = Parameters<typeof createVisionsDispatcher>[0];
type SpawnArgs = Parameters<VisionsSpawn>[0];
...
function spawnAt(seen: SpawnArgs[], index: number): SpawnArgs {
  const args = seen[index];
  if (!args) throw new Error(`no spawn at ${index}`);
  return args;
}
...
test('a COMMENTS verdict whose every entry is junk reports NONE and says why', async (t) => {
  ...
  assert.match(String(result.reason), /survived validation/);
});
...
test('a missing, unparsable, non-object or unknown-verdict file is an ERROR, never a comment', async (t) => {
  ...
  assert.match(String(arrayResult.reason), /not an object/);
...
function dispatcherWithSpawn(spawnSession: VisionsSpawn, overrides: DispatcherOptions = {}) {
...
test('a session that writes the result file yields its comments, and the work dir is removed after', async () => {
  ...
  assert.equal(spawnAt(seen, 0).initialPrompt, VISIONS_BOOTSTRAP_PROMPT);
...
test('hostile buffer bytes stay in the prompt file and never reach spawn arguments', async () => {
  ...
  let promptOnDisk = '';
  const observed: { args: SpawnArgs | null } = { args: null };
  ...
  assert.equal(result.verdict, 'NONE');
  const spawnArgs = observed.args;
  assert.ok(spawnArgs, 'a session was spawned');
...
test('a hung session is aborted at the hard timeout and resolves ERROR, so the lane never pins', async () => {
  ...
  await waitUntil(() => deadline.fire !== null);
  const fireDeadline = deadline.fire;
  assert.ok(fireDeadline, 'the dispatcher armed its hard timeout');
...
test('a timed-out dispatch waits for the killed session before removing its work dir', async () => {
  ...
  const { dispatch, workDirs } = dispatcherWithSpawn(
    () => new Promise<void>((resolve) => { spawnRelease.resolve = resolve; }),
    {
      ...
      clearTimeoutFn: () => {},
      removeWorkDir: async (dir: string) => {
        removals.push(dir);
  ...
  const release = spawnRelease.resolve;
  assert.ok(release, 'the spawn is parked');
  release();
  assert.deepEqual(await pending, {
    verdict: 'ERROR', comments: [], diagnostics: [], intent: null, hand: null, outOfRange: 0, errorSource: 'transport', reason: 'dispatch timed out',
  });
