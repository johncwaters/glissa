...
test('a per-uri push replaces that document and leaves the others alone', async () => {
  ...
  const second = applyFindingsMessage(first, wire({
...
test('the section head names what it actually has, and never pads with a zero', async () => {
  ...
  assert.equal(sectionCountText({ hand: 'whole doc issue', findings: twoFindings, comments: [comment(1, 'x')] }), 'raised hand, 2 findings, 1 comment');
  assert.equal(sectionCountText({ findings: [], comments: [comment(1, 'x'), comment(2, 'y')] }), '2 comments');
  assert.equal(sectionCountText({ findings: [finding(0, 0, 'repeated-word', 'a')], comments: [] }), '1 finding');
...
function thread(id: string | null, text: string, ts = NOW, extra: Partial<IntentThread> = {}): IntentThread {
  return {
