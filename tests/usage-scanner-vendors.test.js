...
function codexTurnContext(model) {
...
function codexTokenCount({ total, last = null, timestamp = '2026-07-08T22:50:28.569Z' }) {
  const info = { total_token_usage: total, model_context_window: 258400 };
...
function claudeLine({ messageId, input = 100, output = 10 }) {
  return JSON.stringify({
...
async function scanOnce(scanner) {
  await scanner.runPass();
  return scanner.buildReport({});
}

async function scannerSessionTotals(root) {
  const scanner = makeScanner(root);
  await scanner.runPass();
