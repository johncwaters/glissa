...
import { decideAutoRebase, AUTO_REBASE_STATES, SPAWN_GAP_TRIGGER } from '../session/core/rebase-gate.ts';
...
const reasonFor = (extra: AutoRebaseInputs) => decideAutoRebase(eligible(extra)).reason;
...
test('guard order holds for the fresh trigger: live-pty sits exactly where busy sits', () => {
  const fresh = (extra: AutoRebaseInputs) => decideAutoRebase(eligible({ trigger: SPAWN_GAP_TRIGGER, state: STATES.STARTING, ...extra }));
