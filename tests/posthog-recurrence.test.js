...
} = require('../server/core/posthog-recurrence');
...
test('findRecurrenceMatch finds the prior transient of the same project', () => {
  ...
  assert.equal(match.key, 'ph.test/1#iss-1');
  assert.equal(match.record.issueId, 'iss-1');
});
...
test('findRecurrenceMatch picks the highest score, then the most recent, deterministically', () => {
  ...
  const match = findRecurrenceMatch({ title: CHUNK_TITLE_B, projectId: 1 }, state, NOW, OPTS);
  assert.equal(match.record.issueId, 'iss-recent', 'equal scores break toward the fresher cluster');
});
...
test('pruneSignatures drops clusters past the window and caps the rest', () => {
  ...
  for (let i = 0; i < SIGNATURE_CAP + 20; i += 1) {
    many[`k${i}`] = { projectId: '1', issueId: `iss-${i}`, title: CHUNK_TITLE_A, lastAt: NOW - i };
  }
