...
import {
  ...
  shouldReplace,
  totalTokensOf,
} from '../server/core/usage-entry-core.ts';
import type { UsageEntry } from '../server/core/usage-entry-core.ts';
...
function fixture(name: string) {
  return fs.readFileSync(path.join(import.meta.dirname, 'fixtures', name), 'utf8').trim();
}
...
function dedupUsageEntries(entries: (UsageEntry | null)[]) {
  const byPrimary = new Map<string, UsageEntry>();
  const byCollision = new Map<string, UsageEntry>();
  const output = new Set<UsageEntry>();
  for (const entry of entries) {
    if (!entry) continue;
    const keys = dedupKeys(entry);
    if (!keys.primary || !keys.collision) continue;
    ...
    const winner = duplicate && existing && !shouldReplace(existing, entry) ? existing : entry;
    if (existing) output.delete(existing);
    output.add(winner);
