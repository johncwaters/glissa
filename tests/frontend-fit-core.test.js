...
test('decideFitAction: the first measured fit repaints and asks the PTY to redraw', async () => {
  const { decideFitAction } = await importCore();
...
test('decideFitAction: a re-borrow at the fitted size sends once without a redraw', async () => {
  ...
  assert.deepEqual(out, { repaint: false, send: true, redraw: false });
});
...
test('decideFitAction: a change on either axis alone is a changed grid', async () => {
  const { decideFitAction } = await importCore();
  const base = { measured: true, lastFittedCols: 150, lastFittedRows: 44, lastSentCols: 150, lastSentRows: 44 };
  assert.deepEqual(
