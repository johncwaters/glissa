...
} from '../server/core/ingest-agent-core.ts';

const NOW = 1700000000000;

function fakeStat({ size, mtimeMs = 1000, ino = 7, birthtimeMs = 500 }: { size: number; mtimeMs?: number; ino?: number; birthtimeMs?: number }) {
...
test('Grok message chunks accumulate and only the completed turn publishes', () => {
  let vendorState: Record<string, string> | null = null;
