...
type AgentLogIngest = ReturnType<typeof createAgentLogIngest>;
...
function inertTimers(): Partial<AgentLogIngestOptions> {
  return {
...
interface RecordingTimers extends Partial<AgentLogIngestOptions> {
...
function countingFs(): { counts: { stat: number; readdir: number }; fsPromises: TranscriptFileSystem } {
  ...
  return {
    ...
    fsPromises: {
      ...
      readdir: (dir, options) => { counts.readdir += 1; return fs.promises.readdir(dir, options); },
    },
  };
}
...
function withHomes(fn: (context: HomesContext) => Promise<void>): () => Promise<void> {
  return async () => {
    ...
    const adapters: AgentLogIngest[] = [];
    const build = (overrides: Partial<AgentLogIngestOptions> = {}): AgentLogIngest => {
...
function seedClaudeTranscript(projects: string, { dirName = 'C--repo', sessionId = 'sess-live', lines = [] }: {
  dirName?: string;
  sessionId?: string;
  lines?: string[];
} = {}): string {
...
test('a completed turn surfaces as an agent-turn without any of the history before it', withHomes(async ({ projects, events, build }) => {
  ...
  await adapter.poll();
  assert.equal(events.length, 0, 'lines written before start() must never publish');
...
test('a transcript created after start is discovered by the next sweep and tailed from its end', withHomes(async ({ projects, events, build }) => {
  ...
  await adapter.poll();
  assert.equal(events.length, 0, 'even a fresh file starts at its end');
...
test('a line split across two polls arrives whole exactly once', withHomes(async ({ projects, events, build }) => {
  ...
  await adapter.poll();
  assert.equal(events.length, 0, 'half a JSON line is not an event');
...
test('a Grok session publishes its completed turn once, not its chunks', withHomes(async ({ grokSessions, events, build }) => {
  ...
  await adapter.poll();
  assert.equal(events.length, 0, 'chunks are streaming, and the plan forbids one event per chunk');
...
test('layer 1, the ledger: a visions-lane transcript in an ordinary project dir is never opened', withHomes(async ({ projects, events, build }) => {
  ...
  await adapter.discover();
  await adapter.poll();
  assert.equal(events.length, 0, 'visions dispatch output must never re-enter the next visions prompt');
}));
...
test('layer 1, the ledger: a record landing AFTER the sweep still stops the lane publishing', withHomes(async ({ projects, events, build }) => {
  ...
  await adapter.poll();
  assert.equal(events.length, 0, 'the per-line check is what covers the record landing late');
}));
...
function watchCapture() {
  ...
  return {
    listeners,
    watchFn: (dir: string, _options: { persistent: boolean }, listener: (eventType: string, fileName: string | null) => void) => {
...
test('an agent event with no root is dropped, because machine scope is the shellHistory contract', withHomes(async ({ codexSessions, events, build }) => {
  ...
  await adapter.poll();
  assert.equal(events.length, 0, 'a rootless agent event would be labelled machine scope in every digest');
}));
...
test('a secret past a long prefix in a tool command is scrubbed whole, never down to its first word', withHomes(async (homes) => {
  ...
  try {
    ...
    if (!tool) throw new Error('no tool event reached the ring');
...
test('a secret past a long prefix in a turn summary is scrubbed whole, and the ring is the only slice', withHomes(async (homes) => {
  ...
  try {
    ...
    if (!secret) throw new Error('the command event never reached the ring');
...
test('lane stop tears the agent-log source down with it', withHomes(async (homes) => {
  ...
  const { lane } = laneWith(homes, { enabled: true, sources: { agentLogs: { enabled: true } } });
  await agentLogsOf(lane).start();
  assert.ok(agentLogsOf(lane).watchCount > 0);
  lane.stop();
