...
// A created worktree always names its branch; the optional field is for the refusal shapes create also
// answers with, which no test here passes to git.
...
test('hasUnmergedWork (real git): uncommitted changes report work; a merged-away branch does not', { skip: !GIT }, async () => {
  ...
  try {
    ...
    git(['merge', '--ff-only', branchOf(ws)], repo); // the operator merged it out of band
    assert.equal(await gw.hasUnmergedWork({ projectPath: repo, workspace: ws, integrationBranch: 'develop' }), false,
