// Guardrail: server/git-workspace.ts is the ONLY module allowed to issue `git worktree`
...
// a bypassing caller racing git-workspace.ts's own worktree add/remove and corrupting a
// concurrent run).
//
// This scans the runtime source tree and fails listing any offender. If you need to
// create, remove, or list a worktree, go through git-workspace.ts instead of shelling to
