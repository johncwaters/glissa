...
import {
  ...
  reviveIntentState,
  sanitizeIntentText,
} from '../server/core/visions-intent-core.ts';
import type { IntentState } from '../server/core/visions-intent-core.ts';
...
function open(
  ...
  text: string,
  { now = NOW, projectId = PROJECT, uri = URI }: { now?: number; projectId?: string | null; uri?: string | null } = {},
) {
