...
const { main, fallbackLine, decodeChainCommand, parsePayload, NO_CHAIN } = require('../session/statusline-relay');
...
test('the POST reaches a listening local server with the raw body intact', async () => {
  ...
  const url = `http://127.0.0.1:${server.address().port}/hook/sess/statusline?t=tok`;
