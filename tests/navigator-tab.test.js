...
/*
 * The Navigator tab's feed, end to end over the REAL sockets: an editor relay on /navigator publishes a
 ...
 * 'navigator-findings' is deliberately NOT replayable: a client that missed a gap is repaired with one
 * 'navigator-snapshot' of the whole map instead, which is the plan-limits precedent and the only shape
...
function withNavigatorBackend(fn) {
  return async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'glissa-navigator-tab-'));
    const cfgPath = path.join(tmpDir, 'config.json');
    fs.writeFileSync(cfgPath, JSON.stringify({
      projects: [], teams: [], repoRoots: [], navigator: { enabled: true },
    }, null, 2), 'utf8');
    ...
    try {
      await fn({ base, track: (ws) => { sockets.push(ws); return ws; } });
    } finally {
...
test('a sweep reaches every connected dashboard, and a later one is repaired by the snapshot', withNavigatorBackend(async ({ base, track }) => {
  const dashboard = await openRecordingSocket(`${base}/control`);
  track(dashboard.ws);
  const snapshotOnFirstConnect = await waitFor(dashboard.received, (msg) => msg.type === 'navigator-snapshot');
  assert.deepEqual(snapshotOnFirstConnect.documents, [], 'nothing is open yet, so the repair frame is empty');

  const relay = await openRecordingSocket(`${base}/navigator`);
  track(relay.ws);
  ...
  const pushed = await waitFor(dashboard.received, (msg) => msg.type === 'navigator-findings');
  ...
  const cleared = await waitFor(dashboard.received, (msg) => msg.type === 'navigator-findings' && msg.diagnostics.length === 0);
  assert.equal(cleared.uri, MARKDOWN_URI);

  const afterClose = await openRecordingSocket(`${base}/control`);
  track(afterClose.ws);
  const emptyRepair = await waitFor(afterClose.received, (msg) => msg.type === 'navigator-snapshot');
...
test('navigator messages are not on the control replay retention list', () => {
  ...
  log.stamp({ type: 'navigator-findings', uri: MARKDOWN_URI, diagnostics: [], ts: 1 });
  log.stamp({ type: 'navigator-snapshot', documents: [], ts: 2 });
