...
// The sender only reads readyState and bufferedAmount, both mutable on this double so a test can move
// the client between drained and wedged.
...
// assert.equal narrows its subject, so an earlier `assert.equal(ws.closed, null)` would leave the later
// read typed as null. Reading through a call returns the declared type instead.
...
// Deterministic scheduler: nothing runs until the test pumps it. The timer handles are this suite's own
// records rather than real Timeouts, so a scheduled close can be inspected instead of waited out.
...
// A distinct NodeJS.Timeout per scheduled callback, minted from a real unref-ed timer that never fires.
