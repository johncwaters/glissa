...
function fakeEngine(results = []) {
  ...
  return {
    ...
    rebaseOnly(args) {
      calls.push(args);
      return queue.length > 1 ? queue.shift() : (queue[0] || { ok: true, rebased: true, headSha: 'newhead' });
    },
  };
}
...
const rebaseEntries = (s) => s.getDebugState().decisions.filter((d) => d.kind === 'rebase');
...
test('never fires while the session is working, waiting, dirty, parked, merging, or switched off', async () => {
  ...
  for (const mergeStatus of ['parked', 'merging']) {
    const engine = fakeEngine();
    const s = makeSession({ engine });
    try {
...
test('a session with no worktree engine never rebases, and an engine failure never breaks the funnel', async () => {
  ...
  const throwing = { rebaseOnly() { throw new Error('engine exploded'); } };
...
test('a conflict and an engine throw each still close the suppressed window with a recheck', async () => {
  for (const result of [{ ok: false, reason: 'rebase-conflict', conflicts: ['a.js'] }, 'throw']) {
    const engine = result === 'throw'
      ? { calls: [], rebaseOnly() { engine.calls.push(1); throw new Error('boom'); } }
      : fakeEngine([result]);
    const s = makeSession({ engine });
    try {
      await s.checkWorktreeChange();
