...
- **Radar error monitoring**: an opt-in lane that polls PostHog error tracking, classifies every issue against the last poll (spiking, regressed, new, quiet), pings Telegram for the ones that matter, and dispatches headless agents that diagnose them into a report and an investigations inbox on the dashboard.
