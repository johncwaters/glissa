...
- **Host terminal stealing focus on every prompt send**: Pressing Enter in a Glissa-managed Claude session raised/flashed the host terminal (e.g. VS Code) on every send. Root cause: Claude Code fires `UserPromptSubmit`, the user's global hook stack spawns `node.exe` grandchildren, and under node-pty's headless OS ConPTY those console spawns allocate a brand-new console window, stealing foreground. Glissa's PTYs now spawn through node-pty's bundled, newer ConPTY (`useConptyDll`) — the same lever VS Code shipped for the identical bug — which keeps full ConPTY fidelity. A new `buildPtyBackendOpts` maps the backend per platform/mode and a `conptyMode` constructor option (default `'dll'`) selects it; if the bundled dll fails to load, `_spawnWithBackendFallback` retries once on the OS ConPTY (guard keyed on `useConptyDll`, so genuine spawn errors still surface as `spawn_fail`), with `'winpty'` as a manual last rung. The fix lives entirely in Glissa's spawn path: no user settings or plugins are touched, and kill/restart is unaffected since Glissa tree-kills via `taskkill /T /F` independent of node-pty.
