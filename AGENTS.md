...
public/
  ...
  phone/           # Phone layout: Board / Terminal / Review / Teams / Radar / PRs screens + bottom nav, rendered only under [data-layout="phone"]
...
- **Server-machine side effects refuse remote callers.** The upgrade handler stamps the connection's trust onto the control `ws` (`ws.glissaTrust`), which is how a handler tells a paired phone from the local dashboard; it comes from the socket's local port like every other trust read, never from anything the client sent. `open-artifact` consults `decideEditorOpenAccess` and answers a remote connection with an explicit error, because opening a run artifact spawns the configured editor ON THE SERVER, where the phone's operator cannot see it, and the old path reported success for a window nobody would look at. Absent trust (remote mode off) reads as local, so local behavior is unchanged.
