...
A pack gathers local context (docs, house rules, curated skills) into a versioned, token-budgeted directory that a session is pointed at with `--add-dir`. Assembly, spawn-time delivery, auto-rebuild, and the live per-turn notice channel so far. A project with no `packs` key spawns byte-identically to before. Plan and milestones in `docs/plan-context-mill.md`.
