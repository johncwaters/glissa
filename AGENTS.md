...
### Context Packs (the context mill: assembly + spawn delivery)

A pack gathers local context (docs, house rules, curated skills) into a versioned, token-budgeted directory that a session is pointed at with `--add-dir`. Assembly, spawn-time delivery, and auto-rebuild so far; the live per-turn notice channel is a later milestone. A project with no `packs` key spawns byte-identically to before. Plan and milestones in `docs/plan-context-mill.md`.
...
- **A rebuild does not touch a live session, and reaches it only halfway.** Skills under an added dir hot-reload mid-session (verified CC behavior), so a rebuilt skill lands in a running session for free; a changed `CLAUDE.md` or `.claude/rules` file does NOT, and reaches only the next spawn. That asymmetry is why the chip exists at all, and why M4's `UserPromptSubmit` notice channel is the piece that closes it.
