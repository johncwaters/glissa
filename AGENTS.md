...
- Codex (`agent: "codex"`) reaches the hook tier: CC-shaped snake_case payloads mapped by a five-row table (`session/adapters/codex.js`, live-verified 0.147.0), hooks injected as `-c 'hooks.<Event>=...'` argv (the only form `exec resume` takes). Two honest gaps: no `Notification` event, so a prose question is indistinguishable from a finished turn, and `backgroundAgents: false` (SubagentStart/Stop firing unverified). Notice, statusLine and anti-slop are capability-off. rtk is ON, on its own `PreToolUse` group through `session/rtk-relay.js`: codex honours an `updatedInput` only beside `permissionDecision: allow` and rtk omits it for some rewrites, so the relay stamps it (`session/core/rtk-hook-core.js`).
- The Grok adapter uses an opt-in home hooks file inert without `GLISSA_HOOK_URL`; injection is refused when operator Claude settings have hooks, `Stop(end_turn)` plus `StopFailure`/`StopCancelled` complete, titles never default ready, and packs use one `--rules` index-pointer token (`session/adapters/grok.js`, `tests/agent-grok.test.js`).
