...
// The two server payloads this panel caches. Both arrive as control-channel replies, so each is cast
// once where it enters (setReviewDiff / setReviewBranchSync) and read as this shape everywhere else.
...
const statusById = new Map<string, string>(); // mergeStatus ('none'|'pending-review'|'merging'|'parked'|'merged')
const reasonById = new Map<string, string | null>();
const diffById = new Map<string, SessionDiffPayload | null>(); // null until fetched
// null while a request is in flight, absent if never requested.
...
export function forgetReviewSession(id: unknown) {
  ...
  if (key === getSelectedId()) { setSelectedId(null); return; } // setSelectedId fires render
  render();
}
