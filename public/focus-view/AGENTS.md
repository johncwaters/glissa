...
- Kept projects: closing the last session of a project keeps its path as an empty rail group (header + "+" quick-add + a dismiss "×") so the operator re-adds a session without re-picking the folder. `focus-view.js` remembers every project path in a `knownProjectPaths` Set (registered by `noteKnownProjectPath`, called from the session lifecycle in `app.js` so it does not depend on the Focus view being active); a known path with no live session renders as an empty group. Persisted via `ui-prefs` `keptProjects` (config.json no longer lists the removed session, so localStorage is the only record it survives a reload on). Membership (not a live/gone transition) drives the empty group; `forgetProject` (the "×") removes a path permanently until a session is added on it again.
- Keep `focus-shortcuts.mjs` in lockstep with the dispatch in `app.js` and the display catalog in `../shortcuts.mjs` when adding a binding.
- `.mjs` files stay pure (no DOM, no window, no localStorage); they run under node:test too.
...
### Internal
- `../session-card/` (cards, activity flags), `../sidebar/selection.js` (focusing selects in the review sidebar), `../app.js` (shortcut dispatch)
