...
// The pill button plus the child refs and wrapper row buildPill caches on it.
type FocusPill = ReturnType<typeof buildPill>;
// A project sublist remembers the id order it last rendered, so an unchanged order skips re-appending.
...
let railHeadEl: HTMLButtonElement | null = null;  // the "{n} NEED YOU" jump header (sticky top of the rail)
...
let active = false;
let attnCursorId: string | null = null;                 // round-robin cursor for the Alt+W attention queue
const mergeStatusById = new Map<string, string>();      // 'none'|'pending-review'|'merging'|'parked'|'merged'
...
const groupListById = new Map<string, GroupList>();   // project key (path) -> its <div role=listbox> sublist
const groupHeaderById = new Map<string, GroupHeader>(); // project key (path) -> its header <div>
...
function ensureGroup(group: RosterGroup<RosterRow>) {
  ...
  query(header, '.focus-rail-group-label').textContent = group.label; // ORIGINAL case; CSS uppercases
...
export function refreshFocusRoster() {
  ...
  for (const group of groups.groups) {
    ...
    mountedRail.appendChild(header); // re-append keeps headers + sublists in sorted order
