...
const buildSection = (title, hint) => buildPanelSection('hooks', title, hint);
...
function buildButton(className, label, onClick, ariaLabel) {
...
/** @returns {string|null} the requestId the message went out with, or null when nothing was sent. */
...
function buildInput(value, placeholder, onInput) {
...
function buildTextarea(value, onInput) {
...
function buildEventSelect(draft, events, onChange) {
...
function buildTypeRow(draft, onChange) {
...
function buildProjectPicker(draft, projects) {
...
function buildTemplateRow(events) {
...
function buildEditor(draft) {
  ...
  const projects = projectsOf(_report);
...
/** @param {Record<string, unknown>|null} hook @param {import('./hooks-view-core.mjs').HookDraft|null} [draft] */
function openEditor(hook, draft = null) {
...
function buildRow(hook, { grouped = false } = {}) {
  const projects = projectsOf(_report);
...
function buildBuiltinRow(entry) {
...
export function setHooksRequestSender(send) {
  _sendRequest = send;
}
...
export function mountHooksView(parent) {
  if (_root) return;
...
export function applyHooksReport(msg) {
...
export function applySaveHookResult(msg) {
  if (!msg || typeof msg !== 'object') return;
  const wasEditorSave = _editorRequestId !== null && msg.requestId === _editorRequestId;
  ...
  const error = typeof msg.error === 'string' && msg.error ? msg.error : 'Could not save the hook';
  if (wasEditorSave) { setEditorError(error); return; }
  showErrorToast(error, { persist: true });
  requestHooksReport();
}

export function applyDeleteHookResult(msg) {
  if (!msg || typeof msg !== 'object') return;
  if (!msg.ok) showErrorToast(typeof msg.error === 'string' && msg.error ? msg.error : 'Could not delete the hook', { persist: true });
  requestHooksReport();
}
