...
// Browsers may ignore requestPermission() outside a user gesture, so the settings toggle (a real
// click) is the reliable path; the boot call is best-effort.
...
// Raise a native notification for a delivered server `notify` message. Same tag per session+category
// dedupes across multiple open tabs and lets an escalation re-fire replace the previous toast.
