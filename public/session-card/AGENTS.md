...
| `terminal.js` | xterm.js setup, data WebSocket, OSC-52 clipboard, key handling (consults `focus-view/focus-shortcuts.mjs` for which Alt+keys bubble) |
...
- Pure cores tested in `tests/frontend-naming.test.js`, `frontend-webgl-pool.test.js`, `frontend-aggregate-status.test.js`; terminal/DOM behavior verified via `npm run dev`.
