...
// An alias, not an interface: it rides the snapshot projection's indexable metadata slot.
type PendingWakeup = {
  at: number | null;
  kind: string;
  reason: string | null;
...
function createSessionBackgroundTracking({
   ...
   * expiry rules off three TTLs), so session/core/agent-tracker.ts owns them together, applies every
  ...
  // (session/core/gate-release.ts) instead of trusting a drained count alone.
  ...
  // session/core/gate-release.ts). Sequence rather than clock: signals share milliseconds.
  let signalSeq = 0;
  ...
  // decideGateRelease (session/core/gate-release.ts) decides whether it may ever fire.
