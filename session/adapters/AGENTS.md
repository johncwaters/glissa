...
- The codex hook-trust bypass is `projects[].codexBypassHookTrust`, config-file only, default OFF, and refused even when opted in if the cwd ancestry holds a `.codex/config.toml` that could contribute hooks or a `.codex/hooks.json` at all. The bypass is not scoped to Glissa's own hooks: it runs every hook the invocation loads (operator config PLUS repo-shipped PLUS agent-written), so it fails toward the title tier (`session/sessions.ts._decideHookTrustBypass`, `tests/agent-codex.test.ts`).
