...
function mapHookToSignal(event: string, payload?: HookPayload): string | null {
  ...
  switch (e) {
    ...
    case "subagentstart":
      ...
      // it is still running does not falsely COMPLETE the card (see session-background-tracking.ts
      // and the activeAgents gate in session/core/status-mapper.ts).
    ...
    case "posttooluse": {
      ...
      // transitions (session-background-tracking.ts). The tool_name switch is defense in depth: if a
