...
| `spawn-env.ts` | Pure `buildAgentEnv(baseEnv, extraEnv, profile)`: the adapter's scrub/set profile applied plus the always-on Glimmervoid marker scrub, optional PATH prepend, returns a copy |
